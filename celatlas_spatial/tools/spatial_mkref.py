...
from celatlas_spatial.tools.__init__ import GENOME_CONFIG
from celatlas_spatial.__init__ import HELP_DICT, __VERSION__
...
class Mkref:
    ...
    def __init__(self, genome_type, args, files=(), non_files=()):
        ...
        self.celatlas_spatial_version = __VERSION__
    ...
    def get_SA(fasta_size):
        """
        Get genomeSAindexNbases from fasta size. For small genomes, the parameter
        --genomeSAindexNbases must be scaled down, with a typical value of min(14, log2(GenomeLength)/2 - 1).
        For example, for 1 megaBase genome, this is equal to 8, for 100 kiloBase genome, this is equal to 7.

        As mentioned in STAR manual, or 1 megaBase genome, this is equal to 9. But when the genome is relatively small,
        e.g. < 3kb, using large value may caused core dump error when mapping.
    ...
    def set_config_dict(self):
        ...
        self.config_dict['celatlas_spatial_version'] = self.celatlas_spatial_version
...
def super_opts(parser, sub_program):
    if sub_program:
        ...
        parser.add_argument('--outdir', help=HELP_DICT['outdir'] + "STAR files", default=".")
