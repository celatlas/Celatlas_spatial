...
import celatlas_spatial
from celatlas_spatial.__init__ import HELP_DICT
from celatlas_spatial.tools.__init__ import FILTERED_MATRIX_DIR_SUFFIX
from celatlas_spatial.tools import utils
from celatlas_spatial.celatlas import ArgFormatter

TOOLS_DIR = os.path.dirname(celatlas_spatial.tools.__file__)
...
def get_conda_env():
    ...
    for path in python_path.split(':'):
        if 'miniconda3/envs/' in path:
            return path.split('miniconda3/envs/')[-1].split('/')[0]
...
class Multi:
    ...
    def __init__(self, assay):
        ...
        self.__CONDA__ = get_conda_env()
        self.__APP__ = 'celatlas_spatial'
    ...
    def common_args(self):
        ...
        parser.add_argument(
            ...
            '''
                ...
                4th column: The 4th column has different meaning for each assay. The single cell rna directory after running Celatlas is called `matched_dir`.
...
'''
    ...
    def binSegment(self, sample):
        step = 'binSegment'
        tif = ''
        model = ''
        bc_input = ''
        count_detail = f'{self.outdir_dic[sample]["count"]}/{sample}_count_detail.txt'
        cmd_line = self.get_cmd_line(step, sample)
        cmd = (
            f'{cmd_line} '
            '--segment --count '
