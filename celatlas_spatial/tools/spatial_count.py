...
from celatlas_spatial.tools.__init__ import (FILTERED_MATRIX_DIR_SUFFIX, RAW_MATRIX_DIR_SUFFIX)
from celatlas_spatial.tools.matrix import COLUMN, ROW
from celatlas_spatial.tools.emptydrop_cr import get_plot_elements
from celatlas_spatial.tools.emptydrop_cr.cell_calling_3 import cell_calling_3
from celatlas_spatial.tools.step import Step, s_common
from celatlas_spatial.rna.mkref import Mkref_rna
from celatlas_spatial.tools.plotly_plot import Line_plot
...
class Count(Step):
    ...
    def run(self):
        ...
        (CB_total_Genes, CB_reads_count, reads_mapped_to_transcriptome) = self.cell_summary(df, cell_bc)
        del df
        ...
        # summary
        self.get_summary(CB_describe, CB_total_Genes, CB_reads_count, reads_mapped_to_transcriptome)
        ...
        line_median = Line_plot(df_line=df_line, title="Median Genes per Cell",
                                x_title=X_TITLE, y_title=MEDIAN_GENE_Y_TITLE).get_plotly_div()
        ...
        while True:
            ...
            for umi_kv in umi_arr:
                ...
                if float(low_count / high_count) > percent:
                    break
                if utils.calc_hamming_distance(low_seq, high_seq) == 1:
    ...
    def discard_read(gene_umi_dict):
        ...
        for umi in umi_gene_dict:
            ...
            gene_id_max = [gene_id for gene_id, read_count in umi_gene_dict[umi].items() if
    ...
    def bam2table(self):
        ...
        with open(self.count_detail_file, 'wt') as fh1:
            ...
            for _, g in groupby(samfile, keyfunc):
                ...
                for gene_id in gene_umi_dict:
                    self.correct_umi(gene_umi_dict[gene_id])

                # output
                for gene_id in gene_umi_dict:
                    for umi in gene_umi_dict[gene_id]:
                        fh1.write(f'{barcode}\t{gene_id}\t{umi}\t{gene_umi_dict[gene_id][umi]}\n')
        samfile.close()
    ...
    def find_threshold(df_sum, idx):
        return int(df_sum.iloc[idx - 1, df_sum.columns.get_loc('UMI')])
    ...
    def sub_sample(fraction, df_cell, cell_read_index):
        """Calculate sequencing saturation metrics using downsampling.

        Sequencing saturation indicates the fraction of library complexity that has been captured.
        Higher saturation means more sequencing yields diminishing returns.

        Formulas:
            sequencing_saturation = 1 - (n_deduped_reads / n_total_reads)
            umi_saturation = Similar but focuses on UMI-level duplicates

        Where:
            n_deduped_reads = Number of unique (barcode, UMI, gene) combinations
            n_total_reads = Total number of reads

        Args:
            fraction: Subsample fraction (0.0 to 1.0)
            df_cell: DataFrame with columns (Barcode, geneID, UMI, count)
            cell_read_index: Array of df_cell row indices repeated by 'count'

        Returns:
            Tuple of (umi_saturation %, read_saturation %, median_genes_per_cell)
        """
        cell_read = df_cell['count'].sum()
        frac_n_read = int(cell_read * fraction)
        subsample_read_index = cell_read_index[:frac_n_read]

        # Get unique UMIs (deduplicated) and their counts
        index_dedup, counts = np.unique(subsample_read_index, return_counts=True)

        # Number of deduplicated reads = number of unique UMIs after subsampling
        n_deduped_reads = len(index_dedup)

        # Total reads in subsample
        n_total_reads = frac_n_read

        # Sequencing saturation = 1 - (unique UMIs / total reads)
        # This represents the fraction of reads that are PCR/optical duplicates
        sequencing_saturation = round((1 - n_deduped_reads / n_total_reads) * 100, 2)

        # For compatibility, keep both metrics the same
        # (In 10X Genomics terminology, this is "Sequencing Saturation")
        umi_saturation = sequencing_saturation
        read_saturation = sequencing_saturation

        # Calculate median genes per cell/barcode
