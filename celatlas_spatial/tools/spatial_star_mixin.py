...
from celatlas_spatial.tools.spatial_mkref import Mkref
...
class Star_mixin(Step):
    ...
    def get_star_metrics(self):
        ...
        with open(self.STAR_map_log, 'r') as map_log:
            ...
            unique_reads_list = []
            multi_reads_list = []
            unmapped_ts_reads_list = []
            unmapped_otr_reads_list = []
            total_reads = 0
            for line in map_log:
                if line.strip() == '':
                    continue
                ...
                if re.search(r'of reads unmapped: too short', line):
                    unmapped_ts_reads_list.append(line.strip().split()[-1])
                if re.search(r'of reads unmapped: other', line):
                    unmapped_otr_reads_list.append(line.strip().split()[-1])
        ...
        unmapped_ts_reads = int(unmapped_ts_reads_list[0])
        unmapped_otr_reads = int(unmapped_otr_reads_list[0])
        ...
        self.add_metric(
            name=f'Unmapped {self.stat_prefix} - too short',
            value=unmapped_ts_reads,
            total=total_reads,
            help_info='reads that did not map to the genome'
        )
        self.add_metric(
            name=f'Unmapped {self.stat_prefix} - other',
            value=unmapped_otr_reads,
            total=total_reads,
            help_info='reads that did not map to the genome'
        )
