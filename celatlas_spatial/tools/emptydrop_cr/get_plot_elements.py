...
from celatlas_spatial.tools.utils import add_log
...
def counter_barcode_rank_plot_data(count_data_path, log_uniform=False):
    ...
    cell_bc = count_data[count_data['mark'] == 'CB'].index
    ...
    non_cell_mask = ~count_data.index.isin(cell_bc)
    first_non_cell = non_cell_mask.argmax() if non_cell_mask.any() else total_bc

    # find the last barcode which is a cell
    cell_mask = count_data.index.isin(cell_bc)
    last_cell = total_bc - 1 - (~cell_mask[::-1]).argmax() if cell_mask.any() else 0

    ranges = [0, first_non_cell, last_cell + 1, total_bc]
    plot_segments = [BarcodeRankPlotSegment(start=0, end=ranges[1], cell_density=1.0, legend=True),
