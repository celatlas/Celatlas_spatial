...
from celatlas_spatial.tools.__init__ import (BARCODE_FILE_NAME, FEATURE_FILE_NAME, MATRIX_FILE_NAME)
...
class Features:
    ...
    def to_tsv(self, tsv_file):
        """
        Write features to tsv file in 10X format (3 columns: gene_id, gene_name, gene_type).
        If gene_type is not provided, default to "Gene Expression".
        """
        if self.gene_type:
            df = pd.DataFrame({'gene_id': self.gene_id, 'gene_name': self.gene_name, 'gene_type': self.gene_type})
        else:
            # Default to "Gene Expression" for compatibility with 10X format
            gene_type = ['Gene Expression'] * len(self.gene_id)
            df = pd.DataFrame({'gene_id': self.gene_id, 'gene_name': self.gene_name, 'gene_type': gene_type})
        df.to_csv(tsv_file, sep='\t', index=False, header=False)

    def to_tsv_gz(self, tsv_file):
        """
        Write features to gzipped tsv file in 10X format (3 columns: gene_id, gene_name, gene_type).
        If gene_type is not provided, default to "Gene Expression".
        """
        if self.gene_type:
            df = pd.DataFrame({'gene_id': self.gene_id, 'gene_name': self.gene_name, 'gene_type': self.gene_type})
            ...
            gene_type = ['Gene Expression'] * len(self.gene_id)
            df = pd.DataFrame({'gene_id': self.gene_id, 'gene_name': self.gene_name, 'gene_type': gene_type})
        df.to_csv(tsv_file, sep='\t', index=False, header=False, compression='gzip')
...
class CountMatrix:
    ...
    def to_matrix_dir(self, matrix_dir):
        ...
        # Write features to uncompressed tsv first, then compress and remove original
        features_path = f'{matrix_dir}/{FEATURE_FILE_NAME}'
        features_path_gz = f'{features_path}.gz'
        self.__features.to_tsv(features_path)
        # Compress features file
        with open(features_path, 'rb') as f_in:
            with gzip.open(features_path_gz, 'wb') as f_out:
                f_out.writelines(f_in)
        # Remove uncompressed features file
        os.remove(features_path)

        # Write barcodes to uncompressed tsv first, then compress and remove original
        barcodes_path = f'{matrix_dir}/{BARCODE_FILE_NAME}'
        barcodes_path_gz = f'{barcodes_path}.gz'
        pd.Series(self.__barcodes).to_csv(barcodes_path, index=False, sep='\t', header=False)
        # Compress barcodes file
        with open(barcodes_path, 'rb') as f_in:
            with gzip.open(barcodes_path_gz, 'wb') as f_out:
                f_out.writelines(f_in)
        # Remove uncompressed barcodes file
        os.remove(barcodes_path)

        # Write matrix as gzipped file
        ...
        with open(matrix_path, 'rb') as f_in:
            with gzip.open(f'{matrix_path}.gz', 'wb') as f_out:
                f_out.writelines(f_in)
        # Remove uncompressed matrix file
    ...
    def from_dataframe(cls, df, features: Features, barcodes=None, row='geneID', column='Barcode', value="UMI"):
        ...
        feature_index_dict = {gene_id: index for index, gene_id in enumerate(features.gene_id)}
        barcode_index_dict = {barcode: index for index, barcode in enumerate(barcodes)}

        # use all barcodes
        barcode_codes = np.array([barcode_index_dict[barcode] for barcode in df.index.get_level_values(level=1)])
        # use all gene_id from features even if it is not in df
        gene_id_codes = np.array([feature_index_dict[gene_id] for gene_id in df.index.get_level_values(level=0)])
